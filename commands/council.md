---
description: Convene the design council (challenger + architecture-reviewer + analyst) when a pre-build loop has stalled — produces a business-and-technical path-forward brief, and either unblocks the build autonomously or hard-stops for a human ruling.
...
- **16(b)** (pre-build `challenger` loop) 2 graded verdicts pass without a `go`, counted by verdict, never by filename, or
...
|---|---|---|
| **What actually breaks** | `challenger` (Absol), in **Stop Brief** mode | What is proven safe, what is genuinely open, what has never been run |
| **Whether the shape should exist** | `architecture-reviewer` | Topology, blast radius, evolution path, cost shape — the question the challenger is forbidden to ask |
| **What each fix does to everything else** | `analyst` (Girafarig) | Upstream and downstream impact of every candidate path; CONTAINS / RELOCATES / WIDENS |

Give all three the same packet: the Stop Brief's candidate paths, the full frozen set, the residual register, and the unrun-verification list. The analyst prices the candidates; the architect rules on shape; the challenger states what is actually open. **No seat may write the fix.**

## The council verdict — computed, not negotiated

**GO** requires all four:
1. No OPEN calibrated blocking HIGH from the challenger (`demonstrated`/`code-traced` + user reach + routine/plausible + money, data, or silent divergence).
2. The architect's verdict is `APPROVE` or `APPROVE-WITH-CONDITIONS` on at least one candidate path.
3. The analyst's verdict on that same path is `SAFE-TO-PATCH` or `PATCH-WITH-CONDITIONS`, and it is **not** `WIDENS`.
