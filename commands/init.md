...
- **Two tiers can both be true at once, and `adr.dir` must carry both when they are.** An **org-tier** ADR repo (Question 1's "Yes" answer — a submodule shared across multiple projects) and a **project-tier** `docs/adr/` folder (ADRs this project alone owns) are not alternatives to pick between — a project can have either, or both, and `adr.dir` is an **array** precisely so it can hold every root that applies, never just the org-tier one. **Never silently drop the project-tier root while adding org-tier domain paths.** The failure mode this guidance exists to prevent: a fullstack/multi-domain project sets `adr.dir` to only the org submodule's per-domain subfolders (e.g. `["adr/infra", "adr/app"]`) and loses `docs/adr/` entirely — any ADR this project needed to own locally silently stops being read, with no error, because the cache just reports fewer roots.
  - **Per-domain org-tier subfolders** (e.g. `adr/infra/` and `adr/app/`, or `docs/adr/software-engineering/` and `docs/adr/infra/`): set `adr.dir` to the array of those subfolders, not the parent, so each domain's ADRs are captured explicitly and visibly.
  - **A local `docs/adr/` folder also exists, or the project expects to start one** (the common case whenever Question 1 was answered "Yes" for org-tier but this project will still author its own project-specific decisions): **append `docs/adr` to that same array** rather than replacing it. Example, org-tier submodule with per-domain subfolders **plus** a project-tier folder:
  ```json
  "adr": { "dir": ["adr/infra", "adr/app", "docs/adr"] }
  ```
  Example, org-tier submodule with no per-domain split, plus project-tier — the default already covers this (`["adr", "docs/adr"]`), so only change it if per-domain splitting is actually needed.
  - If the human confirms this project has no project-tier ADRs at all and never expects to (rare — most projects accumulate at least one local decision), it's fine to omit `docs/adr` from the array. Ask rather than assume: dropping it silently is the bug this note exists to prevent.
  - After building, confirm the `--build` output shows a **non-zero count for every root that's supposed to have content** (e.g. `[…/infra:12, …/app:12, docs/adr:3]`); a `…:0` on a root that's expected to be empty for now (a freshly-added `docs/adr` with nothing in it yet) is fine — a `…:0` on a root that should already hold ADRs means it's empty or misnamed, fix it before proceeding.
...
1. Write `maat.json` with the collected values (at minimum `adr.dir` for your ADR location, plus `adr.autoSync` / `adr.upstreamBranch` if the human enabled sync). Don't ask about `reportStyle` here — it defaults to `"full"` and needs no setup question; mention in the Step 4 summary that adding `"reportStyle": "lean"` to `maat.json` later shortens review-report prose without touching what gets checked or gated (PRINCIPLES.md rule 10), so the human can opt in whenever report volume becomes the thing they want to cut, no re-init required.
...
4. **Create labels, if they don't already exist** (check first: `gh label list --json name`): `blocked-on-owner`, `current-focus` (the small, pinned set of open Issues that genuinely need the human's attention right now — distinct from `blocked-on-owner`: everything `blocked-on-owner` is also `current-focus`, but so is anything a reviewer just flagged HIGH this session, so `gh issue list --label current-focus --state open` alone answers "what needs me right now" without reading `blocked-on-owner` and recent review output separately — see `docs/issue-template.md`), `rule-16-stop` (or this project's own name for its loop-stop rule, per `docs/PRINCIPLES.md`, if it differs), and the verdict set — `verdict:go` / `verdict:no-go` (design-challenger's and the design council's own binary verdict — `agents/design-challenger.md`'s RECEIPT `verdict=go|no-go` and `commands/council.md`'s computed GO/NO-GO — applied to the code PR or issue a round was run against) and `verdict:conditional` / `verdict:reject` (taxonomy only, per this label set's own spec — no agent flow applies these automatically today; `agents/adr-amender.md`'s Recommendation checklist is free-text, not label-producing, and lives in the ADR submodule's own remote, not this repo. Available for a human to apply by hand when triaging, e.g. an ADR-amendment PR's outcome — don't remove them for lacking automation, and don't invent a false producer for them) — plus one label per Feature ID actually discovered in step 3 (none invented if no requirements list exists yet), plus the `chore` catch-all label matching step 3's `Feature ID` field catch-all option (create this one every time, even when no Feature IDs exist, since non-feature work still needs somewhere to land), plus `severity:high` / `severity:med` / `severity:low` (needed by the findings-as-bug-issues flow and by any backlog-to-issue migration that carries a stated severity forward — create all three up front rather than inventing them ad hoc the first time one is needed).

5. **Milestones describe deliverable features, not individual stories — a Milestone always contains multiple stories.** A Milestone is the unit a human checks to see "is this feature done", so it groups every story that has to land before that feature is usable — never one Milestone per story. Only if a delivery-unit plan already exists at init time: check the same requirements/roadmap doc (if found in step 3) for the project's real feature/capability groups (its own requirements taxonomy — a "Groups" or "Feature ID" section, an epic list, a capability map) and any stated target dates.
   - If a feature/capability grouping exists: create one Milestone **per feature**, named from the plan's own language (e.g. "Feature: Policy Engine", not "S4" or "Sprint 4") and due-dated if the plan states one — check first (`gh api repos/:owner/:repo/milestones`), skip any title that already exists, never invent a generic "Sprint 1/2/3" scheme the project's own plan doesn't use, and never create a Milestone per story even if the plan happens to enumerate stories individually — roll each story up into the feature Milestone it belongs to instead.
   - **If the project already has one-Milestone-per-story from before this convention** (a project mid-stream), don't silently restructure it — flag it plainly to the human as a fix worth doing (consolidate into feature-level Milestones, reassign the affected Issues) and let them decide when, same as any other in-progress-project migration in Step 3.6. Don't do the reassignment unasked.
   - **Every story's tracking Issue gets assigned to its feature's Milestone at filing time** (part of the mechanical linkage this step exists to make automatic) — a story with no home yet stays unmilestoned rather than getting a placeholder Milestone invented for it.
   - **If no such plan/feature-grouping exists yet, create no Milestones now** — leave this for the first `/…:story` (or equivalent) to create the relevant feature Milestone on demand once a real feature is defined, rather than inventing a placeholder with no real content.
