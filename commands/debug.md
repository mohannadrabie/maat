...
The debugger reproduces, isolates root cause from symptom, and proposes the minimal fix with a named regression check (before/after output). It is tier-bound: it assigns a risk tier so the right reviewer still reviews the fix, persists its finding to docs/reviews/<scope>-debug-<YYYY-MM-DD>.md as evidence, and never merges.
