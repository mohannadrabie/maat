...
In both modes the manager is read-only and never waives a human-only action, changes a hard rule, or suppresses a security BLOCKER. The manager also owns the **Manager Summary** that every `/review` closes with (verdict · per-reviewer line · blockers+unlocks · conditions · next action). End with the single next action.
