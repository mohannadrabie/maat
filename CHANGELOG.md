...
- **Per-finding evidence tags in every RECEIPT.** PRINCIPLES rule 19 always required each finding to declare `demonstrated` / `code-traced` / `derived`, but the receipt carried only an aggregate tally, which cannot say which finding was derived. All 13 report-producing agents now tag the tier on the finding line, and the `evidence:` line became a checksum over those tags — so "a HIGH may not rest on derived evidence" is checkable exactly instead of inferred.
