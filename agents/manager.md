...
**Cross-domain gaps are the `fullspectrum-reviewer`'s job, not yours to reconstruct from memory (PRINCIPLES.md rule 9).** Each domain reviewer reads only its own slice of the ADR catalog — so no single reviewer sees whether, say, an API change collides with an accepted *data* ADR, or whether two lanes make conflicting assumptions at their boundary. `fullspectrum-reviewer` is the standing pass that closes that gap: it runs on **every review path above TRIVIAL** — including the inline STANDARD single-reviewer case — reads the compressed `adrCatalog.adrs` **whole and unfiltered** (no domain slice), checks the diff's changed files against every applicable ADR OUTSIDE the lane(s) the domain reviewer(s) covered, and hunts non-ADR functional/architectural gaps at the seams between domains. It persists its own dated report and ends its turn with the same `RECEIPT:` shape every other reviewer uses — you audit it exactly like any other reviewer's receipt (reopen triggers, tier-scaled trust), you do not do its job for it. A collision or seam gap it finds is a finding like any other; an ADR collision specifically is a BLOCKER, quoted, resolved only by fix-or-amend. Because it reads the whole catalog once and domain reviewers stay on their slices, the cost stays `1 × whole + N × slice`, not `N × whole`.

**It doesn't count against rule 1's "never more than two reviewers" cap** — it isn't a domain pick, it's the standing seam-check that used to be your own ad hoc read; inviting it alongside the tier's domain reviewer(s) is the same ceremony as before, just done by a receipted specialist instead of an inline guess. It is backstopped at the pre-merge gate too: `/ship-check` independently re-runs a whole-catalog-vs-diff ADR pass, so nothing merges without one even if a standalone `/review` run somehow skipped `fullspectrum-reviewer`.
...
- **Pre-build (`challenger`) loop — rule 16(b).** `roundsSinceLastGo`/`gradedVerdicts`, updated after every graded `challenger` verdict on this artifact. `challenge.md` is the sole writer: it appends each graded verdict and increments (or resets to 0 on a `go`) as its own step. Trips at **2** graded verdicts without a `go`.
...
2. **Collect the challenger's Stop Brief** (frozen set, open calibrated HIGHs, residual register, unrun verifications, 2 to 3 candidate paths with no mechanism design).
3. **Seat all three in parallel** — `challenger` (Stop Brief mode), `architecture-reviewer` (shape), `analyst` (impact of each candidate, upstream and down). Same packet to all three.
...
2. **For everything else**, read the reviewer's stated `Exposure: ~<N>% of <users|requests|runs>, basis: <measured|counted-in-code|assumption>` (PRINCIPLES.md rule 21 requires this line on every `[HIGH]`).
   - `basis: assumption`, or no exposure line at all → **do not bounce it back to the same reviewer that made the vague claim.** Escalate to the `analyst` in its lightweight **exposure-verification mode** (`analyst.md` — a short, mechanical count against the finding itself, not a full six-step candidate-fix analysis). Treat the finding as blocking while this is pending. Once the analyst returns a verified `Exposure:` figure, re-run steps 1-2 above using that real number; if the analyst reports `UNVERIFIABLE`, the finding stays blocking at its cautious default until the named measurement exists. You still make the final blocking-or-not call — the analyst only verifies the number it rests on.
...
2. Pick the minimum reviewer set the tier requires. CRITICAL: `redteam` + the single most-relevant domain reviewer (security / network / architecture / code). Always also seat `fullspectrum-reviewer` — it's the standing cross-domain pass (rule 9), not a domain pick, so it doesn't count against the cap. Never more than the tier warrants beyond that; adding reviewers is a defect, not diligence.
3. Assign each reviewer a DISJOINT scope so no two teammates edit or claim the same files. Reviewers are read-only, so file conflicts are rare, but keep report ownership clean: one reviewer, one dated report. `fullspectrum-reviewer` reads across every teammate's scope by design (that's its job) — it doesn't edit anything, so this doesn't collide.
...
3. **ADR Compliance** — Aggregate ALL ADR findings from all reviewers, **including `fullspectrum-reviewer`'s cross-domain collision check** (rule 9 — the standing whole-catalog pass; domain reviewers only see their own lane)
...
- **Inline:** ONE domain reviewer + `fullspectrum-reviewer` (its standing partner, not an extra pick) + both clean + no BLOCKERS + no ADR violations
- **Spawn manager:** More than one domain reviewer OR any BLOCKER OR any ADR violation OR verdict conflict (including a conflict between the domain reviewer and `fullspectrum-reviewer`)
